    ...
    public class Mem
    {
        ...
        public bool OpenProcess(int pid)
        {
            ...
            try
            {
                ...
                if (theProc.MainModule != null)
                {
                    mainModule = theProc.MainModule;
                    findModule = true;
                }

                getModules(findModule);

                if(mainModule == null)
                {
                    MessageBox.Show("Main Module Not Found.");
                    return false;
                }
        ...
        /// </summary>
        public void getModules(bool found)
        {
            if (theProc == null)
                return;
            ...
            foreach (ProcessModule Module in theProc.Modules)
            {
                if (!string.IsNullOrEmpty(Module.ModuleName) && !modules.ContainsKey(Module.ModuleName))
                {
                    ...
                    if(Module.ModuleName.ToLower().Equals("blackdesert64.exe") && found == false)
                    {
                        mainModule = Module;
                    }
                }
        ...
        private UIntPtr get64bitCode(string name, string path, int size = 16)
        {
            ...
            if (newOffsets.Contains(','))
            {
                ...
                foreach (string oldOffsets in newerOffsets)
                {
                    ...
                    {
                        offsetsList.Add(Int64.Parse(test.Trim()));
                    }
                }
